...
class LaneLine:
    ...
    def __init__(self, lane_side = "Unknow"):
        ...
        self.regression = {"m": None, "b": None} # Current values for simple linear regression
    ...
    def associate_regresion(self, m, b, lines):
        ...
        # Calculate the mean value for each linear regretion parameter
...
def hsv_range_tunner(img_src, tune = True, conf_file_path = "", space_model = cv2.COLOR_BGR2HSV):
    ...
    Returns:
        Arg1Min: `int` Minimun value of first argument in color model (HSV = H / HLS = H)
        Arg2Min: `int` Minimun value of second argument in color model (HSV = S / HLS = L)
        Arg3Min: `int` Minimun value of third argument in color model (HSV = V / HLS = S)
    ...
    if os.path.isfile(conf_file_path):
        ...
        print(bcolors.WARNING + "\tWarning: No hsv configuration file" + bcolors.ENDC)
...
def find_lanelines(img_src, COLOR_TRESH_MIN, COLOR_TRESH_MAX, COLOR_MODEL, HOZ_TRESH = 0.6, FILT_KERN = 5):

    """ Finds a simple linear regression for each lane line (Right and Left)
    Args:
        ...
        HOZ_TRESH: `float` Normalized value to ignore vertical image values
    ...
    for idx in range(0, len(COLOR_MODEL)):
        ...
        img_tresh[0:int(img_tresh.shape[0]*HOZ_TRESH),:] = [0, 0, 0]
    ...
    Ly1 = Ry1 = int(img_src.shape[0]*HOZ_TRESH)
...
def draw_lanelines(img_src, Right_Lane_Line, Left_Lane_Line, HOZ_TRESH = 0.6, draw_lines = True, draw_regression = True, draw_info = True):
    ...
    Args:
        ...
        HOZ_TRESH: `float` normalized vertical value to start printing lines
...
if __name__ == "__main__":
    ...
    # Results folder
    folder_results = "test_videos_output"

    # Get all images names at test folder
    folder_dir_image = "test_images/"
    img_list = os.listdir(folder_dir_image)
    print(img_list)

    #reading an image
    img = cv2.imread(os.path.join(folder_dir_image, 'whiteCarLaneSwitch.jpg'))
    ...
    Tune_ranges = False
    YHmin, YSmin, YVmin, YHmax, YSmax, YVmax = hsv_range_tunner(img_src = img, tune = Tune_ranges, conf_file_path = './yellow_conf_hsv.npz', space_model = cv2.COLOR_BGR2HSV)
    WHmin, WSmin, WVmin, WHmax, WSmax, WVmax = hsv_range_tunner(img_src = img, tune = Tune_ranges, conf_file_path = './white_conf_hsv.npz', space_model = cv2.COLOR_BGR2HSV)
    Hmin, Lmin, Smin, Hmax, Lmax, Smax = hsv_range_tunner(img_src = img, tune = Tune_ranges, conf_file_path = './color_conf_hls.npz', space_model = cv2.COLOR_BGR2HLS)

    # Print tunned parameters
    print("\nYellow:\n\tHmin:{}\tHmax:{}\n\tSmin:{}\tSmax:{}\n\tVmin:{}\tVmax:{}".format(YHmin, YHmax, YSmin, YSmax, YVmin, YVmax))
    print("White :\n\tHmin:{}\tHmax:{}\n\tSmin:{}\tSmax:{}\n\tVmin:{}\tVmax:{}\n".format(WHmin, WSmin, WVmin, WHmax, WSmax, WVmax))
    print("ColorHLS :\n\tHmin:{}\tHmax:{}\n\tLmin:{}\tLmax:{}\n\tSmin:{}\tSmax:{}\n".format(Hmin, Hmax, Lmin, Lmax, Smin, Smax))

    Hmin = Lmin = Smin = Hmax = Lmax = Smax = 0 # TEST
    ...
    COLORHLS_COLOR_LANE_LINE_MIN = np.array([Hmin, Lmin, Smin],np.uint8)  # Minimum color values using HLS color space
    COLORHLS_COLOR_LANE_LINE_MAX = np.array([Hmax, Lmax, Smax],np.uint8)  # Maximum color values using HLS color space

    COLOR_TRESH_MIN = [YELLOW_COLOR_LANE_LINE_MIN, WHITE_COLOR_LANE_LINE_MIN, COLORHLS_COLOR_LANE_LINE_MIN]
    COLOR_TRESH_MAX = [YELLOW_COLOR_LANE_LINE_MAX, WHITE_COLOR_LANE_LINE_MAX, COLORHLS_COLOR_LANE_LINE_MAX]
    COLOR_MODEL = [cv2.COLOR_BGR2HSV, cv2.COLOR_BGR2HSV, cv2.COLOR_BGR2HLS]
